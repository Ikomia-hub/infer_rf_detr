...
from infer_rf_detr.rf_detr.rfdetr.util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, group_detr=1):
        ...
        for g_i in range(group_detr):
            C_g = C_list[g_i]
            indices_g = [linear_sum_assignment(
                c[i]) for i, c in enumerate(C_g.split(sizes, -1))]
            if g_i == 0:
                indices = indices_g
            else:
                indices = [

...
from infer_rf_detr.rf_detr.rfdetr.models.attention import MultiheadAttention
from infer_rf_detr.rf_detr.rfdetr.models.ops.modules import MSDeformAttn
    ...
    for lvl, (H_, W_) in enumerate(spatial_shapes):
        if memory_padding_mask is not None:
            ...
            valid_H = torch.tensor(
                [H_ for _ in range(N_)], device=memory.device)
            valid_W = torch.tensor(
                [W_ for _ in range(N_)], device=memory.device)
...
class Transformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat):
        ...
        if self.two_stage:
            ...
            for g_idx in range(group_detr):
                ...
                if self.bbox_reparam:
                    ...
                    enc_outputs_coord_wh_gidx = enc_outputs_coord_delta_gidx[..., 2:].exp(
                    ) * output_proposals[..., 2:]
                ...
                topk_proposals_gidx = torch.topk(
                    enc_outputs_class_unselected_gidx.max(-1)[0], topk, dim=1)[1]  # bs, nq
        ...
        if self.two_stage:
            if self.bbox_reparam:
                refpoint_embed_cxcy = refpoint_embed[...,
                                                     :2] * refpoint_embed_ts[..., 2:]
                refpoint_embed_cxcy = refpoint_embed_cxcy + \
                    refpoint_embed_ts[..., :2]
                refpoint_embed_wh = refpoint_embed[..., 2:].exp(
                ) * refpoint_embed_ts[..., 2:]
...
class TransformerDecoder(nn.Module):
    ...
    def refpoints_refine(self, refpoints_unsigmoid, new_refpoints_delta):
        if self.bbox_reparam:
            new_refpoints_cxcy = new_refpoints_delta[..., :2] * \
                refpoints_unsigmoid[..., 2:] + refpoints_unsigmoid[..., :2]
            new_refpoints_wh = new_refpoints_delta[..., 2:].exp(
        ...
        def get_reference(refpoints):
            ...
            if self._export:
                query_sine_embed = gen_sineembed_for_position(
                    obj_center, self.d_model / 2)  # bs, nq, 256*2
